...
void MemMgr_HeapInit(uint32_t img_width, uint32_t img_height)
{
	...
	if(DNUM==MASTER_CORE_ID)
	{
		...
		{
			maps_info_ptr->l1_maps_ptr[num_core] = mem_alloc(&msmc_info,l1_width*l1_height*SHORT_SIZE); ///*TODO*/data allignment
			while(maps_info_ptr->l1_maps_ptr[num_core]==NULL);
		}
		for(num_core=0; num_core<NUM_CORES;num_core++)
		{

			maps_info_ptr->l2_maps_ptr[num_core] = mem_alloc(&msmc_info,l2_width*l2_height*SHORT_SIZE);
...
operateLayer1(short* img_ptr, uint32_t w, uint32_t h)
{
	...
	startVal2 = _itoll(TSCH,TSCL);
#endif
        DSP_add16_shift(local_l1_maps_ptr,accum1_ptr,local_l1_maps_ptr,(w*h/4),1);
#ifdef FUNCTION_PROFILE
	endVal2 = _itoll(TSCH,TSCL);
	add1 += ((endVal2-startVal2)/DSP_FREQ_IN_MHZ);
#endif
    }
}
...
operateLayer2(uint32_t w, uint32_t h)
{
    ...
    for(k=0;k<NUM_CORES;k++)
    	DSP_add16_shift((short*)maps_info_ptr->l1_maps_ptr[k],layer1_ptr,layer1_ptr,pixels,1);
#ifdef FUNCTION_PROFILE
	endVal2 = _itoll(TSCH,TSCL);
	add1 += ((endVal2-startVal2)/DSP_FREQ_IN_MHZ);
#endif
	...
	startVal2 = _itoll(TSCH,TSCL);
#endif
    do_padding(layer1_ptr,pad_l2_ptr,w,h,PAD_SIZE,PAD_SIZE);
#ifdef FUNCTION_PROFILE
	endVal2 = _itoll(TSCH,TSCL);
	pad2 += ((endVal2-startVal2)/DSP_FREQ_IN_MHZ);
#endif
	...
	startVal2 = _itoll(TSCH,TSCL);
        ...
        Conv5x5(pad_l2_ptr,layer1_ptr, w, h, &kernel5x5[kern_index][0]);
#ifdef FUNCTION_PROFILE
	endVal2 = _itoll(TSCH,TSCL);
	...
	startVal2 = _itoll(TSCH,TSCL);
#endif
        DSP_add16_shift(local_l2_maps_ptr,accum2_ptr,local_l2_maps_ptr,(w*h/4),1);
#ifdef FUNCTION_PROFILE
	endVal2 = _itoll(TSCH,TSCL);
	add2 += ((endVal2-startVal2)/DSP_FREQ_IN_MHZ);
#endif
    }
}
...
operateLayer3(uint32_t w, uint32_t h)
{
...
#endif
	for(k=0;k<NUM_CORES;k++)
		DSP_add16_shift((short*)maps_info_ptr->l2_maps_ptr[k],layer2_ptr,layer2_ptr,pixels,1);
#ifdef FUNCTION_PROFILE
	endVal2 = _itoll(TSCH,TSCL);
	add2 += ((endVal2-startVal2)/DSP_FREQ_IN_MHZ);
#endif
	...
	startVal2 = _itoll(TSCH,TSCL);
#endif
	do_padding(layer2_ptr,pad_l3_ptr,w,h,PAD_SIZE,PAD_SIZE);
#ifdef FUNCTION_PROFILE
	endVal2 = _itoll(TSCH,TSCL);
	...
	startVal2 = _itoll(TSCH,TSCL);
		...
		Conv5x5(pad_l3_ptr,layer2_ptr, w, h, &kernel5x5[kern_index][0]);
#ifdef FUNCTION_PROFILE
	endVal2 = _itoll(TSCH,TSCL);
